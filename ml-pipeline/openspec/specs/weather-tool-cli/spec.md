# Spec: weather-tool-cli

## Purpose

A command-line interface (`weather-cli`) that invokes the `weather-tool` Lambda function directly via `boto3`, providing human-readable and machine-readable weather output. The CLI is installable as a Python package, available on PATH, and discoverable by AI coding agents via a `SKILL.md` file.
